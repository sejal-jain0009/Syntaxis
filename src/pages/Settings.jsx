    ...
    <div className="dashboard-page settings-page">
      ...
      <main className="dashboard-main">
        ...
        <div className="dashboard-content">
          ...
          <section className="settings-section" aria-labelledby="appearance-title"><div className="settings-section-heading"><p className="dashboard-kicker">APPEARANCE</p><h2 id="appearance-title">Appearance</h2></div><div className="settings-card settings-grid"><SettingSelect label="Theme" value="dark" options={[['dark', 'Dark']]} /><div className="setting-control"><span>Accent</span><div className="accent-options">{[['blue', 'Light blue'], ['mint', 'Mint'], ['coral', 'Coral']].map(([value, label]) => <button className={`accent-swatch ${value} ${settings.accent === value ? 'is-selected' : ''}`} type="button" onClick={() => updateSetting('accent', value)} aria-label={label} aria-pressed={settings.accent === value} key={value}></button>)}</div></div></div></section>
